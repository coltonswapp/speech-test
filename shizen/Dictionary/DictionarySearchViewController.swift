...
//  shizen
//
//  Standalone dictionary search: FTS5-powered live results, tapping opens WordDictionaryDetailView.
//  When the query is empty, shows recent searches stored locally.
//
...
final class DictionarySearchViewController: UIViewController {
    ...
    private var results: [JMDictEntry] = []
    ...
    @objc private func dismissTapped() {
        // UISearchController + keyboard can make `self.dismiss` no-op or dismiss
        // the wrong presentation. Tear down search first, then dismiss the sheet.
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        // Focus the search field without activating the search controller,
        // which would otherwise hide the navigation bar (and the title).
    ...
    private func setupSearchController() {
        ...
        searchController.searchBar.placeholder = "Search kanji, kana, or English…"
    ...
    private func setupTableView() {
        ...
        // A zero-height header suppresses the stray separator drawn above the first row in a plain table.
    ...
    private func applyRecentSearch(_ query: String) {
        ...
        // Setting text does not always trigger updateSearchResults; drive search explicitly.
        runSearch(query)
    }
...
extension DictionarySearchViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        WordDictionaryDetailSheetPresenter.push(surface: entry.expression, from: self)
    }
