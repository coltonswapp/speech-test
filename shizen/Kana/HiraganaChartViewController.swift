//
//  HiraganaChartViewController.swift
//  shizen
//
...
final class HiraganaChartViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let seionChart = KanaGridChartView(kind: .seion, onKanaInteraction: onKanaSelected)
        let voicedChart = KanaGridChartView(kind: .voiced, onKanaInteraction: onKanaSelected)
        let yoonChart = KanaGridChartView(kind: .yoon, onKanaInteraction: onKanaSelected)
        ...
        let yoonCaption = makeCaption(
            "Yōon (\u{3083} · \u{3085} · \u{3087}) — hiragana with small ya · yu · yo sounds"
        )
